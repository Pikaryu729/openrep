...
def test_create_and_list_exercise(client: TestClient):
    response = client.post("/api/exercises", json={"name": "Deadlift", "category": "back"})
...
def test_update_exercise(client: TestClient):
    created = client.post("/api/exercises", json={"name": "Bench Press", "category": "push"}).json()
    response = client.patch(f"/api/exercises/{created['id']}", json={"category": "chest"})
    assert response.status_code == 200
...
def test_delete_exercise(client: TestClient):
    created = client.post(
        "/api/exercises", json={"name": "Overhead Press", "category": "push"}
    ).json()
    response = client.delete(f"/api/exercises/{created['id']}")
    assert response.status_code == 204
    assert client.get(f"/api/exercises/{created['id']}").status_code == 404


def test_create_duplicate_name_409(client: TestClient):
    assert client.post("/api/exercises", json={"name": "Deadlift"}).status_code == 201
    response = client.post("/api/exercises", json={"name": "Deadlift"})
...
def test_rename_onto_existing_name_409(client: TestClient):
    client.post("/api/exercises", json={"name": "Deadlift"})
    other = client.post("/api/exercises", json={"name": "Bench Press"}).json()
    response = client.patch(f"/api/exercises/{other['id']}", json={"name": "Deadlift"})
    assert response.status_code == 409


def test_delete_exercise_in_use_409(client: TestClient):
    exercise_id = client.post("/api/exercises", json={"name": "Back Squat"}).json()["id"]
    ...
    assert response.status_code == 409
    assert client.get(f"/api/exercises/{exercise_id}").status_code == 200
